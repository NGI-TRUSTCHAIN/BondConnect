...
export async function callContractMethod(
    ...
    export async function initContractsServiceAdmin(_logger: Logger, _contracts: ContractCollection, _config: Config, _originNetwork: string) {
        logger = _logger;
        contracts = _contracts;
        config = _config;
        signer = new ethers.Wallet(config.NETWORK.ADMIN_ACCOUNTS_PRIV_KEY).connect(new ethers.JsonRpcProvider(manageNetwork(_originNetwork)));
